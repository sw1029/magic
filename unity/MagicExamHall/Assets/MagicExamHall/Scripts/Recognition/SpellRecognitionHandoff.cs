...
namespace MagicExamHall
{
    ...
    public sealed class SpellRecognitionHandoff
    {
        ...
        private string DefaultBaseReason()
        {
            ...
            return $"{SpellLabels.Korean(targetFamily)} 문양으로 확정하지 않았습니다.";
        }

        private string DefaultBaseHint()
        {
            return status == RecognitionStatus.Recognized
                ? "좋습니다. 같은 문양을 유지하면 다음 시험으로 넘어갈 수 있습니다."
                : $"{SpellLabels.Korean(targetFamily)}의 큰 실루엣을 먼저 맞춰 다시 그려 보세요.";
        }
        ...
        private string DefaultOverlayReason()
        {
            ...
            {
                return $"{SpellLabels.Korean(recognizedOperator.Value)} 장식으로 인식되었습니다.";
            }

            return "장식 후보를 seal에 안정적으로 붙이지 못했습니다.";
        }
    }
}
