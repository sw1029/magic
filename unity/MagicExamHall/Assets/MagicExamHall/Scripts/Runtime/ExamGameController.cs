...
namespace MagicExamHall
{
    public sealed class ExamGameController : MonoBehaviour
    {
        ...
        private void BuildUi()
        {
            ...
            var reportHeading = CreateText("Report Heading", reportPanel, "입학 복구 보고서", 28, FontStyle.Bold, new Vector2(35, -32), new Vector2(870, 42), Anchor.TopLeft);
    ...
    public sealed class EndingReport
    {
        ...
        public string BuildText(int totalAttempts, string outputDirectory, bool trueEnding, int completedFinalGoals, int totalFinalGoals, IReadOnlyList<string> noteExcerpts)
        {
            ...
            return
                ...
                $"도달 상태  {endingName}\n" +
                "────────────────────────────────────────\n" +
                $"전체 시도 {totalAttempts}회    발견 {discoveries.Count}개    퀘스트 {questChecklistCompleted}/{questChecklistTotal}\n" +
                $"가장 많이 사용한 base  {favoriteBase}    가장 많이 사용한 overlay  {favoriteOverlay}\n" +
                ...
                "보정 정책: 성공/실패 판정은 유지하고 품질 설명과 다음 연습 방향에만 사용합니다.\n" +
                BuildReflectionLine(favoriteBase, favoriteOverlay, discoveries.Count) + "\n" +
                (trueEnding ? "탑의 별자리에 당신의 이름이 기록되었습니다." : "탑은 당신의 문양을 다음 시험까지 기억합니다.") + "\n\n" +
                "자기 평가  손에 잘 맞은 문양 / 다음에 고칠 점 / 예측 가능한 조합\n" +
